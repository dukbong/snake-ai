...
> 외부 RL 라이브러리 없이 PPO 알고리즘을 밑바닥부터 구현하고,
> 멀티프로세스 병렬 환경으로 학습을 최적화한 Snake 게임 AI입니다.
...
단순히 라이브러리를 가져다 쓰는 것이 아니라, 강화학습 알고리즘을 직접 구현하며 내부 동작을 이해하는 것이 목표였습니다.

초기에는 **11차원 상태 표현 + DQN** 방식으로 시작했습니다.
...
Schulman et al. (2017) 논문을 기반으로 직접 구현했습니다.
...
- **Entropy Bonus**: 조기 수렴 방지 및 탐험 유지
- **done / truncated 분리**: 타임아웃과 실제 종료를 구분해 GAE 계산 오류 방지
- **LR Linear Annealing**: 학습 진행에 따라 학습률 선형 감소

### CNN Actor-Critic 네트워크
게임 상태를 픽셀 대신 **7채널 의미론적 그리드(24×32)**로 표현합니다.
...
### 멀티프로세스 병렬 환경
`multiprocessing.Process` + `Pipe` 기반 `SubprocVecEnv`를 직접 구현했습니다.

- 8개 환경이 각각 독립 프로세스에서 동시 실행
- 데이터 다양성 확보 및 학습 효율 향상
- `fork` 이전 PyTorch 초기화를 방지하는 생성 순서 보장
...
방향성 보상(±0.01)은 sparse reward 문제를 완화해 초기 학습 속도를 높입니다.
...
`n_steps=256`으로 늘린 3차에서 롤아웃이 길어져 장기 전략 학습이 개선됐습니다.
